...
void MSVDisplayHelper::displayLine(SBPosition3 start, SBPosition3 end, std::string text) {
...
void MSVDisplayHelper::displayLine(ublas::vector<double> center, ublas::vector<double> dir, int length)
{
...
void MSVDisplayHelper::displayLengthText(SBPosition3 start, SBPosition3 end, std::string text) {
...
void MSVDisplayHelper::displayDirectedCylinder(SBPosition3 start, SBPosition3 end)
{
...
void MSVDisplayHelper::displayDirectedCylinder(SBPosition3 start, SBPosition3 end, float * color, int radius) {
  unsigned int nCylinders = 1;
  unsigned int nPositions = 2 * nCylinders;
  unsigned int* indexData = new unsigned int[2 * nCylinders];
  float *positionData = new float[3 * nPositions];
...
void MSVDisplayHelper::displayPlane(SBVector3 vec, SBPosition3 shift) {
  ...
  //
  //  ublas::vector<double> dir = ANTVectorMath::CrossProduct(posB - posA, posC - posA);
  //
...
void MSVDisplayHelper::displayTriangleMesh(ANTPolyhedron * p)
{
...
void MSVDisplayHelper::displayPolyhedron(ANTPart * part)
{
  ...
  ANTAuxiliary::ANTArray<float> positionData = ANTAuxiliary::ANTArray<float>(3, nPositions);
  ANTAuxiliary::ANTArray<float> radiusData = ANTAuxiliary::ANTArray<float>(nPositions);
  ANTAuxiliary::ANTArray<unsigned int> flagData = ANTAuxiliary::ANTArray<unsigned int>(nPositions);
  ANTAuxiliary::ANTArray<float> colorData = ANTAuxiliary::ANTArray<float>(4, nPositions);
  ANTAuxiliary::ANTArray<unsigned int> capData = ANTAuxiliary::ANTArray<unsigned int>(nPositions);

  ANTAuxiliary::ANTArray<unsigned int> indexData = ANTAuxiliary::ANTArray<unsigned int>(nCylinders * 2);
...
void MSVDisplayHelper::displayOrthoPlane(SBVector3 vec, SBPosition3 shift)
{
...
void MSVDisplayHelper::displayInfoPanel(ANTPart* part)
{
  float * color = new float[4];
  color[0] = 1.0f;
  color[1] = 1.0f;
  ...
  string * text = new string[numFields];

  text[0] = "nucleotides: " + to_string(part->GetNumberOfNucleotides());
  text[1] = "single strands: " + to_string(part->GetSingleStrands().size());
  text[2] = "edges: " + to_string(part->GetEdges().size());
  text[3] = "vertices: " + to_string(part->GetVertices().size());
  text[4] = "faces: " + to_string(part->GetFaces().size());

  QPoint mousePos = SAMSON::getMousePositionInViewport();
 
  QPoint displacement = QPoint(0, 30);
  for (int i = 0; i < numFields; ++i) {
    SBPosition3 pos = SAMSON::getWorldPositionFromViewportPosition(mousePos + i * displacement);
...
void MSVDisplayHelper::displaySphere(SBPosition3 pos, float radius)
{
  ANTAuxiliary::ANTArray<float> positions = ANTAuxiliary::ANTArray<float>(3);
  ANTAuxiliary::ANTArray<float> radiiV = ANTAuxiliary::ANTArray<float>(1);
  ANTAuxiliary::ANTArray<float> colorsV = ANTAuxiliary::ANTArray<float>(4);
  ANTAuxiliary::ANTArray<unsigned int> flags = ANTAuxiliary::ANTArray<unsigned int>(1);
...
void MSVDisplayHelper::displayVector(SBVector3 vec, SBPosition3 shift)
{
  int length = 1000;
  vec *= length;
...
void MSVDisplayHelper::displayVector(SBVector3 vec, SBPosition3 shift, float * color, int length) {
  vec *= length;
...
void MSVDisplayHelper::displayArrow(SBVector3 vec, SBPosition3 start)
{
...
void MSVDisplayHelper::displayArrow(SBPosition3 start, SBPosition3 end, unsigned int nodeIndex, float * color, bool selectable)
{
