...
ANTPart* DASEditor::CreateTube(size_t length, SBPosition3 start, SBVector3 direction) {
...
ANTPart * DASEditor::CreateRoundOrigami(SBQuantity::length diameter, SBPosition3 center, SBVector3 direction, int min_length) {
...
ANTPart * DASEditor::CreateTwoTubes(size_t length, SBPosition3 start, SBVector3 direction, SBVector3 sepDir) {
  ...
  SBQuantity::length sepDist = SBQuantity::angstrom(0.5*ANTConstants::DH_DIST + 5*ANTConstants::DH_DIAMETER);
  SBPosition3 startFirst = start - sepDist * sepDir;
  SBPosition3 startSecond = start + sepDist * sepDir + SBQuantity::nanometer(length * ANTConstants::BP_RISE) * direction;
...
void DASEditor::AddDoubleStrandToANTPart(ANTPart * part, size_t length, SBPosition3 start, SBVector3 direction) {
  if (part != nullptr) {
